...
| ACTIVE_PI_EMPLOYEE_ID      | TEXT/128  | Y        | Employee ID of Active PI. A grant system may return more than one PI, but in PASS there can only be one PI per grant (there can be many Co-PIs, but only one PI). This value will be used to select by Employee ID which user is the PI in such cases.                                         |
| ACTIVE_PI_INSTITUTIONAL_ID | TEXT/128  | Y        | Institutional User ID of Active PI. A grant system may return more than one PI, but in PASS there can only be one PI per grant (there can be many Co-PIs, but only one PI). This value will be used to select by Institutional ID which user is the PI in such cases.                          |
